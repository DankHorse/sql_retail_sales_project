...
GROUP BY shift
```
11. **Find the total sales amount and the number of orders for each product category.**:
```sql
SELECT SUM(total_sale), quantity, category FROM retail_sales
GROUP BY quantity, category
ORDER BY category;
```
12. **Analyze average sales  across different customer age groups and genders. Take the ages between 20 and 30.**:
```sql
SELECT AVG(total_sale), age, gender FROM retail_sales
